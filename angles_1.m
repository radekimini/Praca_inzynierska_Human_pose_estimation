...
% Pętla rysująca z dodatkowym obliczaniem kątów:
...
for vf = 1:total_output_frames
    ...
    % Oś pionowa (vertical axis)
    p7 = pW_cartesian{7}(frame_idx,:);
    p10= pW_cartesian{10}(frame_idx,:);
    v_vert = p10 - p7; v_vert = v_vert ./ norm(v_vert);

    % Kąty względem osi pionowej
    angles_vert = zeros(4,1);
    for k = 1:4
        a = pairs_vert(k,1); b = pairs_vert(k,2);
        u = pW_cartesian{b}(frame_idx,:) - pW_cartesian{a}(frame_idx,:);
        angles_vert(k) = atan2(norm(cross(u,v_vert)), dot(u,v_vert));
    end

    % Kąty ugięcia pomiędzy segmentami
    ...
    for k = 1:4
        ...
        angles_joint(k) = atan2(norm(cross(u1,u2)), dot(u1,u2));
    end

    % Rysowanie na klatce: segment‑vs‑vertical i ugięcia
    for k = 1:4
        txt1 = sprintf('Seg %d-%d vs vert: %.1f°', pairs_vert(k,1), pairs_vert(k,2), rad2deg(angles_vert(k)));
        txt2 = sprintf('Joint %d-%d-%d: %.1f°', joint_triplets(k,:), rad2deg(angles_joint(k)));
        text(10, 20 + 20*(k-1), txt1, 'Color','y','FontSize',9);
        text(200,20 + 20*(k-1), txt2, 'Color','c','FontSize',9);
    end

    % Rysowanie skeleton jak wcześniej
    points2D = nan(19,2);
    for i = 1:19
